...
prompt_file = BASE_DIR / "prompts" / "BRAND_ALLERGIES_prod.txt"
...
async def get_brand_allergens(description, price_description): 
    ...
    if response.response.startswith("```json"): 
        ...
        return json.loads("".join(nutrition_text[1:-1])), compute_time
    else: 
        return json.loads(response.response), compute_time
