...
WAIT_TIME = 0.5 #when clicling or performing an action, playwright needs some type to update the DOM
PROD_CONCURRENCY = 1 
...
DB_PATH = BASE_DIR / "databases" / "df_products.csv"
...
async def get_items(page: Page,subcategory: str): 
    ...
    for k in range(subsection_count): 
        ...
        for z in range(items_count): 
            try: #if a product doesn't contain that button it means that the product is out of stock
                await section_items.nth(z).locator("[data-testid='open-product-detail']").click()
            except: 
                print("Skipping an out of stock product")
                continue
            await asyncio.sleep(WAIT_TIME)
